...
class Product{
    ...
    // read one record for that database
    function getDetails(){

        $query = "SELECT name, price, description, image, category_id FROM " . $this->table_name . " WHERE id = ? LIMIT 0,1";
    ...
    //Returns the total row count by search value
        ...
        if($this->image){
            ...
            $file_upload_error_messages="";
             ...
             if(!in_array($file_type, $allowed_file_type)){
                 $file_upload_error_message .= '<div> Only JPG, JPEG, PNG, and GIF files are allowed.</div>';
             }

             //make sure file does not exist
             if(file_exists($target_file)){
                 $file_upload_error_message .= '<div> Image already exists. Try to change file name.</div>';
             }

             // make sure the submitted file is not too large, can not be larger then 1 MB
             if($_FILES['image']['size'] > (1024000)){
                 $file_upload_error_message .= '<div>Image must be less then 1 MB in size.</div>';
             }

             // make sure the 'uploads' folder exists
             // if not, create it
             if(!is_dir($target_directory)){
                 mkdir($target_directory, 0655, true);
             }
             ...
             if(empty($file_upload_error_message)){
                ...
                if(move_uploaded_file($_FILES['image']['tmp_name'], $target_file)){
                    ...
                    echo "<div class='alert alert-danger'>";
                        echo "<div>Unable to upload photo.</dev>";
                        echo "<div>Update the record the record to upload photo.</div>";
                    echo "</div>";
                }
            }else{
